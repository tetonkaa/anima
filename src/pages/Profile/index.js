...
export default function Profile(props) {
  ...
  return (
    <div className="profilePageContainer">
      <h1>Welcome {props.user.displayName ? props.user.displayName : ""}</h1>
      {props.user.photoURL ? (
        <img className="profilePic" src={props.user.photoURL}></img>
      ) : (
        ""
      )}
      <h1>A list of your creations</h1>
      <div className="flex flex-row profileCardContainer">
        {tests.length > 0 ? (
          tests.map((test, i) => {
            return (
              <div className="mainCard w-80">
                <Link
                  to="/test"
                  key={test._id}
                  href="#"
                ...
                >
                  <div class="max-w-sm bg-none rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <a href="#">
                      <img src={test.testPic} alt="" />
                    </a>
                    <div class="p-5 w-95">
                      <a href="#">
                        <h5 class="overflow-wrap-normal mb-2 text-2xl font-bold tracking-tight text-white-900 dark:text-white">
                          {test.testName}
                        </h5>
                      </a>
          ...
          <div className="testsPlaceholder">
            <p>Your Tests will display here</p>
          </div>
        )}
      </div>
    </div>
